...
class SystemMemoryMonitor {
    ...
    func getCurrentMemoryStatus() -> MemoryStatus {
        ...
        let aiRecommendation = getAIRecommendation(
            availableMemory: memInfo.available, pressureLevel: pressureLevel)
    ...
    func isAISafeToRun() -> Bool {
        ...
        return status.pressureLevel != .critical
            && status.availableMemory > MemoryThresholds.lowMemoryWarning
    }
    ...
    private func getSystemMemoryInfo() -> (total: Double, available: Double, used: Double) {
        ...
        let totalMemory =
            Double(stats.free_count + stats.active_count + stats.inactive_count + stats.wire_count)
            * Double(pageSize) / (1024 * 1024 * 1024)
        ...
        let availableMemory =
            Double(stats.free_count + stats.inactive_count) * Double(pageSize)
            / (1024 * 1024 * 1024)
        ...
        if AppLog.isVerboseEnabled {
            AppLog.debug(
                "Memory calc: total=\(String(format: "%.1f", boundedTotal)) avail=\(String(format: "%.1f", boundedAvailable)) used=\(String(format: "%.1f", boundedUsed)) GB"
            )
        }
    ...
    private func getAIRecommendation(availableMemory: Double, pressureLevel: MemoryPressureLevel)
        -> AIMemoryRecommendation
    {
    ...
    private func startMemoryPressureMonitoring() {
        memoryPressureSource = DispatchSource.makeMemoryPressureSource(
            eventMask: [.normal, .warning, .critical], queue: DispatchQueue.global(qos: .background)
        )
        ...
        memoryPressureSource?.setEventHandler { [weak self] in
            ...
            if AppLog.isVerboseEnabled {
                AppLog.debug(
                    "Memory pressure: \(status.pressureLevel.rawValue) (\(String(format: "%.1f", status.availableMemory))GB avail)"
                )
            }
...
extension SystemMemoryMonitor {
    ...
    func logMemoryStatus() {
        ...
        if AppLog.isVerboseEnabled {
            AppLog.debug(
                "Memory Report: total=\(String(format: "%.1f", status.totalMemory)) avail=\(String(format: "%.1f", status.availableMemory)) used=\(String(format: "%.1f", status.usedMemory)) pressure=\(status.pressureLevel.rawValue) rec=\(status.aiRecommendation.rawValue) quant=\(self.getRecommendedQuantization().rawValue)"
            )
        }
    }
    ...
    func isMemoryUsageHealthy() -> (healthy: Bool, reason: String) {
        ...
        if status.availableMemory < MemoryThresholds.lowMemoryWarning {
            return (
                false, "Low memory available (\(String(format: "%.1f", status.availableMemory))GB)"
            )
        }
        ...
        if status.usedMemory / status.totalMemory > 0.9 {
            return (
                false,
                "High memory usage (\(String(format: "%.0f", (status.usedMemory / status.totalMemory) * 100))%)"
            )
        }
