...
class CoreDataStack: ObservableObject {
        ...
        container.loadPersistentStores { storeDescription, error in
            if let error = error {
                ...
                if AppLog.isVerboseEnabled {
                    AppLog.debug(
                        "Core Data store loaded: \(storeDescription.url?.lastPathComponent ?? "unknown")"
                    )
                }
            }
        }
    ...
    private func createManagedObjectModel() -> NSManagedObjectModel {
        ...
        folderEntity.properties.append(contentsOf: [
            folderBookmarksRelationship, folderParentRelationship, folderSubfoldersRelationship,
        ])
