...
class PasswordManager: NSObject, ObservableObject {
    ...
    override init() {
        ...
        self.encryptionKey = PasswordManager.generateOrRetrieveEncryptionKey()
        super.init()
        loadSavedPasswords()
        loadSettings()
    }
    ...
    private static func generateOrRetrieveEncryptionKey() -> SymmetricKey {
        return PasswordManager.createEncryptionKey()
    }
    
    private static func createEncryptionKey() -> SymmetricKey {
        let serviceName = "com.web.browser.passwords"
