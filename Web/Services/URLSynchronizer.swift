...
class URLSynchronizer: ObservableObject {
    ...
    func updateFromWebViewNavigation(
        url: URL?, title: String?, isLoading: Bool, progress: Double, tabID: UUID
    ) {
    ...
    func updateFromTabSwitch(
        tabID: UUID, url: URL?, title: String, isLoading: Bool, progress: Double, isHibernated: Bool
    ) {
        urlUpdateQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    private func performURLUpdate(
        url: URL?, title: String?, isLoading: Bool, progress: Double, source: URLUpdateSource
    ) {
    ...
    private func generateDisplayURL(from urlString: String, title: String?, source: URLUpdateSource)
        -> String
    {
