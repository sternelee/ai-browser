...
struct MixedContentWarningView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    technicalDetailRow(
                        "Current Policy", value: mixedContentManager.mixedContentPolicy.rawValue)

                    if let mixedStatus = mixedContentManager.getMixedContentStatus(for: tabID) {
                        technicalDetailRow(
                            "Mixed Content Status",
                            value: mixedStatus.mixedContentDetected ? "Detected" : "None")
                        technicalDetailRow(
                            "Only Secure Content",
                            value: mixedStatus.hasOnlySecureContent ? "Yes" : "No")
                        if mixedStatus.violationCount > 0 {
                            technicalDetailRow(
                                "Violations", value: String(mixedStatus.violationCount))
                        }
                    }
                }
            ...
            if warningType != .blocked {
                HStack {
                    Button(action: {
                        ...
                        HStack(spacing: 8) {
                            Image(
                                systemName: userAcknowledgedRisk
    ...
    private var warningMessage: String {
        ...
        case .blocked:
            return
                "This website tried to load insecure HTTP content on a secure HTTPS page. The browser has automatically blocked this content to protect your security."
        case .warning:
            return
                "This secure HTTPS website is trying to load content over insecure HTTP connections. This creates a security vulnerability that could allow attackers to intercept or modify the content."
        case .allowWithRisk:
            return
                "You are about to allow insecure content on a secure page. This significantly reduces the security of this website and could expose you to security risks."
        }
    }
    ...
    private var technicalExplanation: String {
        ...
        case .blocked:
            return
                "The browser's mixed content policy automatically blocked HTTP resources from loading on this HTTPS page. This prevents potential security vulnerabilities while maintaining the page's secure context."
        case .warning:
            return
                "Mixed content occurs when a secure HTTPS page attempts to load resources (images, scripts, stylesheets) over insecure HTTP. This breaks the security guarantee that HTTPS provides."
        case .allowWithRisk:
            return
                "Allowing mixed content will downgrade the security of this page. Any data transmitted to HTTP resources will not be encrypted and could be intercepted by attackers on the network."
        }
    }
